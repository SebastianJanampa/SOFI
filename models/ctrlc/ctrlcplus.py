...
class GPTran(nn.Module):
    def __init__(self, backbone, transformer, num_queries, 
                 aux_loss=False, use_structure_tensor=True, mixed=True):
        ...
        self.n_samples = 16
        self.line_features = nn.Conv2d(self.n_samples, 1, kernel_size=1)
        line_dim = 6        
    ...
    def sampling_points(self, endpoints, n_samples=16):
        ...
        return grid #samples.permute(0, 3, 2, 1).contiguous()
...
#     def forward(self, samples: NestedTensor):
        ...
        samples = F.grid_sample(features, grid, align_corners=False)
        samples = samples.permute(0, 3, 2, 1).contiguous()
        queries = self.line_features(samples).squeeze(1)

        # vlines [bs, n, 3]
        lines = self._to_structure_tensor(lines_)

        queries += self.input_line_proj(lines)

        hs, memory, enc_attn, dec_self_attn, dec_cross_attn = (
            self.transformer(src=features, mask=mask,
                             query_embed=self.query_embed.weight,
                             tgt=queries, #self.input_line_proj(lines), 
...
def build(args):
    ...
    losses = ['zvp', 'fovy','hl', 'hline_labels', 'vline_labels']   
