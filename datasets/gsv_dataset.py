...
class GSVDataset(Dataset):
    def __init__(self, args, listpath, basepath, return_masks=False, transform=None):
...
def build_gsv(image_set, args):
    root = './data/google_street_view_191210/manhattan/'

    PATHS = {
        "train": 'data_csv/gsv_train_20210313.csv',
        "val":   'data_csv/gsv_val_20210313.csv',
        "test":  'data_csv/gsv_test_20210313.csv',
    }

    img_folder = root
    ann_file = PATHS[image_set]
    dataset = GSVDataset(args, ann_file, img_folder, 
                         return_masks=args.masks, transform=make_transform(image_set))
    return dataset
