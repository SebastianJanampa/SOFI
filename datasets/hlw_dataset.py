...
def build_hlw(image_set, args):
    assert image_set == 'test', print("HLW dataset only support for validation")
    
    root = './data/hlw/images/'

    PATHS = {
        "test": 'data_csv/hlw_test.csv',
    }

    img_folder = root
    ann_file = PATHS[image_set]
    dataset = HLWDataset(args, ann_file, img_folder, 
                         return_masks=args.masks, transform=make_transform())
    return dataset
