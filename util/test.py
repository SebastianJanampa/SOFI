...
def test_with_aux(model, data_loader_test, device, args):
...
#             pred_hl_pts = compute_hl_np(pred_hl, img_sz)
            ...
            with open(csvpath, 'a', newline='') as csvfile:
                csvwriter = csv.writer(csvfile)
                L = [filename, err_angle, err_pitch, err_roll, err_fovy, err_hl]
                for angle, pitch, roll, fovy, hl in zip(list_err_angle, list_err_pitch,
                                                        list_err_roll, list_err_fovy,
                                                        list_err_hl):
                    L.extend([angle, pitch, roll, fovy, hl])
                # csvwriter.writerow([filename,                                    
